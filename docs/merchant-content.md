...
The one-time product-background setup completed on 2026-09-13 through the Admin API, with a backup, compare-and-set protection and read-back. The merchant approved `write_products` access and the image-only definition was created. The setup script was retired after its initial CSS approximation was superseded by the exact Figma artwork below; it is not a recurring maintenance command. Future changes use the shared Admin client and current product data.
...
## Collection discovery — 2026-09-15

Existing product `custom.hide_in_collection` is now respected by the shared collection catalog. New Collection boolean `custom.hide_in_filter_navigation` hides only a collection’s all-products filter, not products or its landing page. Leave blank/false to show it. New Collection `custom.featured_usp` references the existing `usp_item` definition and supplies the optional larger hero badge before `custom.usp_set`. Soda points to the active `soda-no-added-sugar` entry with its uploaded full-frame vector artwork. Both definitions and the link were created/read back with the Admin GraphQL client; no browser editing was used. The badge's intentionally English caption follows the supplied artwork. Existing source translations, USP set order, product visibility, collection membership and template assignments were preserved.

## Navigation imagery and hierarchy

Header menus own labels, order and destinations. Nest root → category → resource links for mega panels. For a Page, Collection, Blog post or Blog, set **Navigation image** (`custom.navigation_image`) on that resource. These image-only definitions are provisioned and pinned; upload/select the image there and maintain its focal point in Files. Collection/Blog post native images are fallbacks. Product cards keep **Teaser image → Featured image**. Header image blocks were removed; missing imagery renders a text card. External/custom links currently remain text-only; a URL + image metaobject is the recommended optional extension. See [Header](sections/header.md).
Hard Seltzer backgrounds (2026-09-15): the existing product `custom.gallery_gradient` fields now match Figma collection cards `10938:86677` (Maracuja: five-stop orange/cream) and `10938:86723` (Holunder: five-stop green). Variety Pack `10938:86761` uses its warm base gradient as fallback plus the exact background-only orange/green composition in `custom.gallery_background_image`, matching the Soda Variety workflow. Export: 907×1372 PNG, `seltzer-variety-background-figma.png`, MediaImage `73627879309699`. The two blurred shape layers, fills and transforms are preserved; cans, labels, shadows and rounded corners are excluded. A temporary export frame was removed after download; the source design was unchanged. Admin API compare-and-set writes populated only the previously empty fields on these three canonical products, with an ignored local backup and readback verification. Other Seltzer products and existing Soda values were not changed. These store-owned backgrounds feed the shared collection/navigation/PDP renderer; Product Overview's existing section-owned lighting controls remain separate.
