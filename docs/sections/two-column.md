...
Add **2 column** under Brand storytelling. Two static, editor-selectable groups keep ownership clear: **Content** and **Sidebar**. Editors add, remove and reorder children inside each group. The groups themselves keep content first and sidebar second.

Content supports Text (heading and rich text), Image, Text and image, Poster and FAQ accordion. Sidebar supports repeatable **Sidebar Box** and **Link Box**. No artificial one-box limit is imposed; Shopify's native template/block limits still apply. Native theme blocks are used instead of copying section implementations into a monolithic switch.
...
Poster uses `poster-content` and the same stylesheet/motion controller as the standalone section, including native nested tick items. FAQ uses `faq-list-content`, `faq-item`, `component-faq.css` and the existing `faq-accordion` controller; see [FAQ accordion](faq-accordion.md). The main group supports independent heading font choices per block. Erode is the default; Newake is optional; body remains Maison Neue. Main headings use Section, sidebar headings Card, links Compact and paragraphs Body roles.
...
Block picker previews: Sidebar Box and Link Box load their shared editorial stylesheet directly, so Shopify can render them in isolation without the parent 2 column section. The storefront and picker use the same component styles, including the caret and rounded surface.
