...
Product background setup (2026-09-13): app version `content-api-2` requests `write_products` in addition to existing file/metaobject scopes. Shopify installation consent was completed and installed `write_products` / `read_products` scopes verified on 2026-09-13. The one-time product-background setup then created the image definition and updated the Soda Variety Pack gradient. Its obsolete approximation script was subsequently retired; use the shared Admin client for future authorized maintenance. Releasing app configuration alone does not grant new scopes; always verify installed scopes before store-data work.

Navigation access setup (2026-09-15): app version `content-api-navigation` requests `write_online_store_navigation` for the approved copy from `main-nav-soda` to `general-main-nav` and `main-nav-seltzer`. Merchant consent and installed read/write navigation scopes were verified. The ordinary app launch initially redirected to its placeholder homepage without presenting consent; opening `https://admin.shopify.com/store/sparklys-hard-seltzer/oauth/install?client_id=470179b7453ea19a451b7cdede84ca72` presented the update. Reuse the managed installation URL for future declared scope updates and verify granted scopes through the API before writes. The approved menu copy to `main-nav-general` (actual General menu handle) and `main-nav-seltzer` completed with API read-back; source and targets were backed up to ignored `.shopify/menu-copy-1789475207690.json`. Menu identities and titles were preserved.

Navigation image provisioning (2026-09-15): `content-api-navigation-images` adds `write_content`; merchant consent and installed access were confirmed before creating Page, Collection, Article and Blog `custom.navigation_image` definitions. Use `node scripts/setup-navigation-images.mjs` for a read-only plan and add `--apply` only for authorized provisioning. It preserves existing definitions and resource values, creates only missing image-reference definitions, and verifies each result. The managed installation URL above successfully presented the scope approval.
