...
Product background setup (2026-09-13): app version `content-api-2` requests `write_products` in addition to existing file/metaobject scopes. Shopify installation consent was completed and installed `write_products` / `read_products` scopes verified on 2026-09-13. The product-background setup script then created the image definition and updated the Soda Variety Pack gradient successfully. Releasing app configuration alone does not grant new scopes; always verify installed scopes before store-data work.
